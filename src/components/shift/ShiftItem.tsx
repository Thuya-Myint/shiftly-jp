...
export const ShiftItem = memo(function ShiftItem({ shift, theme, baseLang, onDelete, onUpdate, primaryColors }: {
  ...
  return (
    <div
      key={shift.id}
      className={cn(
        "group relative overflow-hidden rounded-[2.5rem] p-6 sm:p-8 shadow-2xl cursor-pointer border",
        theme === 'light'
      ...
      <div className="flex justify-between items-start relative z-10 gap-6">
        <div className="flex flex-col gap-4 flex-1">
          <div className="flex flex-wrap items-center gap-3">
            <span
              className={cn(
                "text-[10px] font-bold uppercase tracking-[0.2em] px-4 py-2 rounded-xl shadow-sm",
            ...
            <span
              className="text-[10px] font-bold tracking-widest px-4 py-2 rounded-xl bg-gray-100/50 dark:bg-white/5 text-gray-500 dark:text-gray-400 shadow-sm border border-gray-200/30 dark:border-white/5"
            >
              {shift.shift_date}
            </span>
            ...
            <button
              ...
              className={cn(
                "text-[10px] font-bold px-4 py-2 rounded-xl border-2 active:scale-95",
          ...
          <div className="flex items-center gap-4">
            <div className={cn("p-2 rounded-lg", theme === 'light' ? 'bg-gray-100' : 'bg-white/5', primaryColors.text)}>
              <Clock size={20} strokeWidth={2} />
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">{shift.start_time}</span>
              <span className={cn("text-lg font-medium opacity-30", primaryColors.text)}>
                →
              </span>
              <span className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">{shift.end_time}</span>
            </div>
          </div>
        ...
        <div className="flex flex-col items-end gap-3 flex-shrink-0">
          <div
            className={cn(
              "px-5 py-3 rounded-2xl shadow-lg border-t border-white/10",
              primaryColors.bgGradient
            )}
          >
            <p className="text-xl sm:text-2xl font-bold text-white tracking-tight">{yen.format(shift.pay)}</p>
          </div>
